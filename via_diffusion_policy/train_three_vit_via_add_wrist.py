...
vision_encoder_right = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14')
vision_encoder_right = vision_encoder_right.to(device)

vision_encoder_left = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14')
vision_encoder_left = vision_encoder_left.to(device)
    ...
    for epoch_idx in tglobal:
        ...
        if (epoch_idx) % CONFIG["validate_every_epoch"] ==0:
            ...
            for _ in range(int(len(val_data['main-image'])/8) -2 ):
                ...
                with torch.no_grad():
                    ...
                    right_image_features = ema_nets_unwrapped['vision_encoder_right'].forward_features(nright_images)
                    right_image_features = right_image_features["x_norm_clstoken"] # [1, 256, 384])

                    left_image_features = ema_nets_unwrapped['vision_encoder_left'].forward_features(nleft_images)
        ...
        with tqdm(dataloader, desc='Batch', leave=False) as tepoch:
            for nbatch in tepoch:
                ...
                right_image_features = nets['vision_encoder_right'].forward_features(n_right_image)
                right_image_features = right_image_features["x_norm_clstoken"]

                left_image_features = nets['vision_encoder_left'].forward_features(n_left_image)
                left_image_features = left_image_features["x_norm_clstoken"]
