...
def unnormalize_imagenet(image):
    """Undo ImageNet normalization and convert RGB → BGR for correct display."""
    mean = np.array([0.485, 0.456, 0.406]).reshape(3, 1, 1)
    std  = np.array([0.229, 0.224, 0.225]).reshape(3, 1, 1)
    image = image * std + mean  # Back to RGB
    image = image[[2, 1, 0], :, :]  # RGB → BGR
    return np.clip(image, 0, 1)


def visualize_main_images(dataloader, n_samples=4):
    """Visualizes the first observation frame of a few samples from main-image."""
    sample_batch = next(iter(dataloader))
    sample_images = sample_batch["main-image"][:n_samples]  # (B, T, 3, H, W)
    
    B, T, C, H, W = sample_images.shape
    print(f"Visualizing {n_samples} samples, each with {T} time steps...")

    plt.figure(figsize=(4 * n_samples, 4))
    for i in range(n_samples):
        img = sample_images[i, 0].cpu().numpy()  # Take first frame of each sequence
        img = unnormalize_imagenet(img)  # (3, H, W)
    ...
    plt.tight_layout()
    plt.suptitle("First Timestep of main-image", y=1.05)
    plt.show()

# visualize
visualize_main_images(dataloader)
    ...
    for epoch_idx in tglobal:
        ...
        with tqdm(dataloader, desc='Batch', leave=False) as tepoch:
            for nbatch in tepoch:
                ...
                # n_right_image = nbatch['right-image'][:,:CONFIG["obs_horizon"]orizon].to(device)
                ...
                # concat with low-dim observations
                obs_features = torch.cat([main_image_features, nagent_pos], dim=-1)
                obs_cond = obs_features#.unsqueeze(0).flatten(start_dim=1)
