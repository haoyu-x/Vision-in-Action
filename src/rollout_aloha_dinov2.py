...
class Rollout:
    def __init__(self):
        ...
        self.sub_top_RGB = rospy.Subscriber("/camera/top_RGB", CompressedImage, self.top_rgb_callback)
        ...
        # pot
        # model_name = 'teddy_aloha_shared_rgb_dinov2_cls_pot_lime_ema_100.pth'
        # train_dataset_path= '/home/haoyux/teddy_data/pot_lime_rgb_train.zarr.zip'
        ...
        model_name = 'teddy_aloha_share_rgb_dinov2_cls_bag_ema_500.pth'
        train_dataset_path= '/home/haoyux/teddy_data/bag_train.zarr.zip'


        # shelf
        # model_name = 'teddy_aloha_shared_rgb_dinov2_shared_cls_ema_500.pth'
        # train_dataset_path= '/home/haoyux/teddy_data/cup_train.zarr.zip'
        ...
        train_data = {
            ...
            'agent_pos': np.array(loaded_dateset["proprio"]).astype(np.float32)[:, 7:],
            'action': np.array(loaded_dateset["action"]).astype(np.float32)[:, 7:]
        }
        ...
        while not rospy.is_shutdown():
            ...
            while start_flag:        
                ...
                main_image = np.stack([x['top_RGB_arr'] for x in self.obs_deque])
                ...
                with torch.no_grad():
                    ...
                    right_image_features = nets['vision_encoder'].forward_features(nright_images)
                    right_image_features = right_image_features["x_norm_clstoken"] # [1, 256, 384])
                   
                    left_image_features = nets['vision_encoder'].forward_features(nleft_images)
                    left_image_features = left_image_features["x_norm_clstoken"] # [1, 256, 384])
                    ...
                    # obs_features = torch.cat([main_image_features, right_image_features, left_image_features], dim=1)
                    # obs_features = nets['transformer_encoder'](obs_features)
                    ...
                    # obs_cond =  torch.cat([obs_features, nagent_poses], dim=1)
    ...
    def _get_obs(self):
        ...
        while True:
            ...
            proprio_arr = np.hstack((right_proprio_arr, left_proprio_arr))
            ...
            obs["right_RGB_arr"] = np.array(self.right_rgb)       
            obs["left_RGB_arr"] = np.array(self.left_rgb)
            obs["top_RGB_arr"] = np.array(self.top_rgb)
    ...
    def top_rgb_callback(self, data):
        ...
        self.top_rgb = cv2.resize(rgb_image, (224, 224))
    ...
    def right_rgb_callback(self, msg):
        ...
        self.right_rgb = cv2.resize(rgb_image, (224, 224))
    ...
    def left_rgb_callback(self, msg):
        ...
        self.left_rgb = cv2.resize(rgb_image, (224, 224))
    ...
    def load_model(self, action_dim, obs_horizon, lowdim_obs_dim, load_dir, model_name):
        ...
        head_dinov2_vits14 = head_dinov2_vits14.cuda()


        # right_dinov2_vits14 = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14')
        # right_dinov2_vits14 = right_dinov2_vits14.cuda()


        # left_dinov2_vits14 = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14')
        # left_dinov2_vits14 = left_dinov2_vits14.cuda()
        ...
        nets = nn.ModuleDict({
            ...
            # 'right_vision_encoder': right_dinov2_vits14,
            # 'left_vision_encoder': left_dinov2_vits14,
